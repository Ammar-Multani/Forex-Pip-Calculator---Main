...
/**
 * Generate PDF for web - opens a new window with printable content
 */
const generateWebPdf = async (
  ...
  const htmlContent = `
    ...
    <html>
      <head>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no" />
        <title>Forex Calculation Results</title>
        <style>
          body {
            font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;
            margin: 0;
  ...
  try {
    // For web, open the HTML in a new window
    ...
    if (newWindow) {
      ...
      Alert.alert(
        "Error",
        "Unable to open a new window. Please check your browser settings."
      );
      return null;
    }
  } catch (error) {
...
export const sharePdf = async (filePath: string): Promise<void> => {
  // Skip sharing on web as the content is already displayed in a new window
  if (Platform.OS === "web") {
    return;
  }
  ...
  try {
    ...
    Alert.alert(
      "Error",
      "An error occurred while sharing the PDF. Please try again.",
      [{ text: "OK" }]
    );
  }
};
